...
MODEL_NAME = "RCNN_DS_BS" + str(BATCH_SIZE) + '_F' + FUNCTION + '_E' + str(EPOCHS) + '_N' + str(NNEURONS)  + '_NL' + str(NLAYER)
...
def load_training_data(path):
	...
	with open(path + "/train/data.csv", 'r') as data_file:
		...
		for line in data_file:
			...
			image_path = line[0]
			train_images.append(cv2.imread(path + "/train/" + image_path))
	...
	with open(path + "/valid/data.csv", 'r') as data_file:
		...
		for line in data_file:
			line = line.strip().split(',')
			image_path = line[0]
			train_images.append(cv2.imread(path + "/valid/" + image_path))
			train_labels.append([int(line[3])])
...
def load_testing_data(path):
	...
	with open(path + "/test/data.csv", 'r') as data_file:
		...
		for line in data_file:
			line = line.strip().split(',')
			image_path = line[0]
			test_images.append(cv2.imread(path + "/test/" + image_path))
...
def load_validation_data(path):
	...
	with open(path + "/test/data.csv", 'r') as data_file:
		...
		for line in data_file:
			line = line.strip().split(',')
			image_path = line[0]
			valid_images.append(cv2.imread(path + "/test/" + image_path))
...
print(MODEL_NAME, SAVING_PATH + MODEL_NAME)
## Load old model
